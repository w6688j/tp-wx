...
indexObj.ajaxSsc = function(issue, id) {
	...
	var iferror = false;
	var key = $('#lotKey').val();//开奖类型

	$.ajax({
		// url: "/hall/hallajax/getLotteryInfo?lotKey="+key,
		url: "http://www.pkbird.com/Home/api/getssc",
		//url: urlbublic + "CQShiCai/getBaseCQShiCai.do?issue=" + issue,
		type: "GET",
		data: {
		},
		beforeSend: function() {
			if(animateID[id] == undefined) {
                animate.sscAnimate(id); //请求执行开奖动画
			}
		},
