...
var firstTopDisLayer = cc.Layer.extend({
    ...
    g_hostIp = "http://"+window.location.host+"/Home/Api/getPk10?t="+ Math.random(),
	...
	pk10 = cc.Layer.extend({
		...
		rest:function () {
            ...
            $.getJSON(g_hostIp, function(a) {
                ...
                b._curPed = a.current.periodNumber;
                b._nextPed = a.next.periodNumber;
                if (null != a.next.awardNumbers || "" != a.next.awardNumbers) {
                    var c = [],
                        c = a.current.awardNumbers.split(",");
                    b._btmLayer.setCurData(c);
                    g_isAddMaskSpr && b.initCarMask(c)
                }
                var c = a.next.awardTimeInterval,
                    f = a.next.awardTimeInterval / 1E3,
                    f = f + Number(a.next.delayTimeInterval);
                // console.log(_timeNumber);
                b._timeNumber = f.toFixed(0);
                0 < c ? (f += 6, b._timeNumber = f.toFixed(0), b._timeLable.setString(b._timeNumber), b.schedule(b.timeLableRunning, 1)): (6<f?b._timeNumber = f:b._timeNumber = 6, b._timeLable.setString(b._timeNumber), b.schedule(b.timeLableRunning, 1))
            });
        },
		timeLableRunning: function() {
			// console.log(this._timeNumber);
			if (0 < this._timeNumber){
               if(this._timeNumber%10==0){this.rest()}
	...
	MidActionLayer = cc.Layer.extend({
		...
		initCarSprite: function(a) {
			...
			b = cc.Sprite.create("http://"+window.location.host+"/Public/main/img/res/rank3.png");
			...
			b = cc.Sprite.create("http://"+window.location.host+"/Public/main/img/res/rank2.png");
			b.setPosition(cc.p(120, 350));
			...
			a = cc.Sprite.create("http://"+window.location.host+"/Public/main/img/res/rank1.png");
...
var ParticleDemo = cc.LayerGradient.extend({
	...
	DemoFlower = ParticleDemo.extend({
		onEnter: function() {
			...
			a = cc.TextureCache.getInstance().addImage("http://"+window.location.host+"/Public/main/img/res/ffff.png");
