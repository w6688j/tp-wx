...
//                $data['type'] = "bj28";
	...
	public function message(){
		...
		$count = $message->where(array('type'=>$game))->count();
		$page = new \Think\Page($count,10);
		$show = $page->show();
		$list = $message->limit($page->firstRow.','.$page->listRows)->order("id ASC")->where(array('type'=>$game))->select();
		$this->assign('list',$list);
		$this->assign('show',$show);
	...
	public function add_message(){
		...
		if(IS_POST){
			if(!IS_AJAX){
				...
				if($res){
					$this->success('添加成功',U('Admin/Robot/add_message/game/'.$game),1);
				}else{
					$this->error('添加失败');
				}
			}
	...
	public function edit_message(){
		...
		if(IS_POST){
			if(!IS_AJAX){
				...
				if($res){
					$this->success('修改成功',U('Admin/Robot/message/game/'.$game),1);
				}else{
					$this->error('修改失败');
				}
			}
	...
	public function kefu(){
		if(IS_POST){
			if(!IS_AJAX){
                ...
                $result = M('config')->where(array('id'=>1))->save($data);
				if($result){
					$this->success('修改成功');
				}else{
					$this->error('修改失败');
    ...
    public function yinghangka(){
        if(IS_POST){
            $uname=I('uname');
            $bankdress=I('bankdress');
            $banknum=I('banknum');
            if (empty($uname) || empty($bankdress) || empty($banknum)){
                $this->error('参数不能为空');
            }
            $info['bank_data']=I('uname').','.I('bankdress').','.I('banknum');
            $result = M('config')->where(array('id'=>1))->save($info);
            if($result){
                $this->success('修改成功');
            }else{
                $this->error('修改失败');
