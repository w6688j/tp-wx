...
//        10期结果
        $list = M('sscnumber')->where("game = 'ssc'")->order("time DESC")->limit(10)->select();
        // 创建SDK实例
...
//        dump($data);
    ...
    public function getjilu(){
        ...
        $userinfo = session('user');
        $pkdata = F('getssc');
        $jilu  = M('order')->where(array('userid'=>$userinfo['id'],'game'=>'ssc','state'=>1))->limit(20)->order('time desc')->select();
...
//                $this->ajaxReturn('44');
    ...
    public function ssc_header(){
        $this->display();
    }
    public function ssc_header2(){
        $this->display();
    }
}