...
class JscarController extends BaseController{

    public function index(){
        //10期结果
        $list = M('number')->order("time DESC")->limit(10)->where("game= 'jscar'")->select();
        // 创建SDK实例
        ...
        $lists = M('message')->order("id DESC")->where("game = 'jscar'")->limit(20)->select();
        //记录开始
...
//        $this->assign('jilu',$jilu);
    ...
    public function getjilu(){
        ...
        $userinfo = session('user');
        $pkdata = F('getjscar');
        $jilu  = M('order')->where(array('userid'=>$userinfo['id'],'game'=>'jscar','state'=>1))->limit(20)->order('time desc')->select();
    ...
    public function jincai(){
        //聊天信息
        $list = M('message')->order("id DESC")->where("game = 'jscar'")->limit(20)->select();
        $this->assign('list',$list);
        $this->display();
    }
    /*客服*/
