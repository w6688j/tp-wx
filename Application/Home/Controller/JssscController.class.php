...
class JssscController extends BaseController
{

    public function index(){
//        10期结果
        $list = M('sscnumber')->where("game = 'jsssc'")->order("time DESC")->limit(10)->select();
        // 创建SDK实例
...
//        dump($data);
    ...
    public function getjilu(){
        ...
        $userinfo = session('user');
        $pkdata = F('getjsssc');
        $jilu  = M('order')->where(array('userid'=>$userinfo['id'],'game'=>'jsssc','state'=>1))->limit(20)->order('time desc')->select();
